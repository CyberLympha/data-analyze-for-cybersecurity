SET_employee_def_salary = 50_000
SET_employee_def_currency = 'rub'
SET_employee_age_limit = 75
SET_employee_call_text = 'уже идет к вам.'
...
SET_manager_premium = 0.09
SET_engineer_premium = 0.1

class Employee:
    '''Базовый класс для работы с сотрудниками'''
    __AGE_LIMITS = SET_employee_age_limit
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if type(age) != int or age > SET_age_high or age < SET_age_low:
    ...
    def __init__(
            self, name, surname, 
            position, age, 
            salary=SET_employee_def_salary, currency=SET_employee_def_currency
            ):
        """
    ...
    def convert_currency(value,
        ...
        exchange_rate = {
            'rub': 1,
            'usd': SET_exchange_rate_USD,
            'eur': SET_exchange_rate_EUR
        }

        # Пересчет курса валют
        rate = exchange_rate[cur_currency] * exchange_rate[req_currency]
    ...
    def __call__(self):
        ...
        """
        print(f"Сотрудник {self.name} {SET_employee_call_text}")
...
class Manager(Employee):
    ...
    def give_premium(self):
        premium = sum([engineer.salary for engineer in self.engineers]) * SET_manager_premium
        print(f"{self.position} {self.name} {self.surname} получил премию в размере {premium} {self.currency}")
        return sum([engineer.salary for engineer in self.engineers]) * SET_engineer_premium
